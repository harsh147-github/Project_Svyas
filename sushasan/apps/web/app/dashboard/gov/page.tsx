...
import { getGovDashboardData } from '@/lib/gov-from-db'
import { LoopClose } from '@/components/gov/LoopClose'
...
export default async function GovPage() {
  const { wards, solutions } = await getGovDashboardData()
  const sorted = [...solutions].sort((a, b) => b.priority_score - a.priority_score)

  const byWard = sorted.reduce<Record<string, typeof sorted>>((acc, s) => {
                    ...
                    <div key={s.id} className="px-6 py-4 flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${STATUS_COLORS[s.status] ?? STATUS_COLORS.open}`}
                          >
                            {STATUS_LABELS[s.status] ?? s.status}
                          </span>
                          {s.budget_feasible && (
                            <span className="text-[10px] text-india-green font-medium">✓ Budget feasible</span>
                          )}
                        </div>
                      ...
                      <Link
                        ...
                        className="flex-shrink-0 self-start px-3 py-1.5 bg-saffron/10 border border-saffron/30 text-saffron text-xs font-medium rounded hover:bg-saffron/20 transition-colors"
                      >
                        View ward
                      </Link>
                    </div>
