...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const deadSources = bySource
      ...
      : []
    // Distinguishes "the classify event was never sent" (config gap, e.g.
    // INNGEST_EVENT_KEY unset) from "it was sent but the worker didn't keep
    // up" — the two look identical from raw_posts/posts counts alone.
    const classifyTrigger = lastRunRow?.errors?.classify ?? null
    ...
    return noStore({
      ...
      checks: {
        ...
        classification: classifyHealthy
          ? (classifyKeepingPace === false
              ? (classifyTrigger && classifyTrigger.attempted && !classifyTrigger.triggered
                  ? `❌ classify event NOT sent on last run — ${classifyTrigger.reason ?? 'unknown reason'}`
