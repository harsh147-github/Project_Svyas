...
Public social chatter (Twitter, Reddit, Instagram, Telegram, Google Maps, local news)
  ↓
AI scraping + classification (Claude Sonnet — per post)
  ↓
Ward-level hotspot map (citizens see this — what's broken near me)
  ↓
AI solution synthesis (Claude Opus — step-by-step, budgeted, actionable)
  ↓
...
| Vector store | pgvector on Supabase — 1024-dim voyage-3 embeddings |
...
| Embeddings | `voyage-3` multilingual |
...
**Stage 1 — Classify** (Sonnet, every 60 min via Inngest)
Each scraped post → `issue_tag` (traffic/water/electricity/garbage/other), `severity` (1–5), `sentiment` (-2 to 2), `cited_location`, `is_actionable`, `ward_id`, voyage-3 embedding.

**Stage 2 — Cluster** (cosine similarity ≥ 0.85, same ward + same issue_tag)
Groups similar reports. Sonnet generates `centroid_text` (1 neutral sentence summary).

**Stage 3 — Synthesize** (Opus, Sunday 21:00 IST + on-demand)
